...
(function () {
  ...
  function update() {
    ...
    for (var i=0;i<imgEls.length;i++){
      ...
      imgEls[i].style.opacity = (colOp*(dims[i]||1)).toFixed(3);
      var par=-p*vh*(pfs[i]||0.1);
      imgEls[i].style.transform = "translate("+tx.toFixed(1)+"px,"+(ty+par).toFixed(1)+"px) scale("+sc.toFixed(3)+")";
    }
    if (headEl){ var hh=clamp((p-0.30)/0.22,0,1); headEl.style.opacity=hh.toFixed(3); headEl.style.transform="translateY(-50%) scale("+lerp(1.05,1,eo(hh)).toFixed(3)+")"; }
