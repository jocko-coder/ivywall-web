...
(function () {
  ...
  function rupd(){
    ...
    for(var i=0;i<n;i++){
      ...
      if(t>=0){
        var r=t>3?3:t;
        ty=-(r*vh*0.085); sc=1-r*0.075; br=18+r*22;
        op=r>2.6?Math.max(0,1-(r-2.6)/0.6):1;
        z=100-Math.round(r*12);
        fl="brightness("+(1-r*0.12).toFixed(3)+")"+(r>0.3?" blur("+Math.min(r*1.1,3).toFixed(2)+"px)":"");
      } else {
        var u=(-t)>3?3:(-t);
        ty=u*vh*0.058; sc=1-u*0.05; br=18+u*6;
        op=u>2.4?Math.max(0,1-(u-2.4)/0.6):1;
        z=100-Math.round(u*12);
        fl=u>0.05?"brightness("+(1-u*0.045).toFixed(3)+")":"none";
      }
      var c=cardEls[i];
