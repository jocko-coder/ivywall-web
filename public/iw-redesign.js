...
(function () {
  ...
  function build() {
    ...
    try{ var io=new IntersectionObserver(function(es){es.forEach(function(e){if(e.isIntersecting)s.classList.add("bpin");});},{threshold:0.04}); io.observe(s);
  ...
  function update() {
    ...
    var colOp = clamp((p-0.02)/0.14,0,1);
    for (var i=0;i<imgEls.length;i++){
      var rp = eo(clamp((p-0.04 - i*0.02)/0.66,0,1));
...
(function(){
 ...
 function fz(){ if(!footEl||!footEl.isConnected){ var sec=document.getElementById("iwend"); footEl=sec?sec.querySelector(".ie-footwrap"):null; } if(!footEl) return;
   ...
   footEl.style.transform="scale("+(1-prog*0.02).toFixed(3)+")";
   footEl.style.borderRadius=(prog*8).toFixed(1)+"px";
 }
