...
(function () {
  ...
  var IMAGES = [
    { img:"/photos/BWPlus_Ivywall_21_Boodle_Platter.jpg",    css:"top:26%;left:5%;width:clamp(180px,23vw,400px);height:clamp(220px,40vh,440px);z-index:6;",  ox:120, oy:0,   tag:"Boodle <b>feast</b>" },
    { img:"/photos/BWPlus_Ivywall_10_Signature_Seafood.jpg", css:"top:9%;right:7%;width:clamp(150px,19vw,330px);height:clamp(120px,28vh,300px);z-index:5;",  ox:-90, oy:80,  tag:"Seafood <b>paella</b>" },
    { img:"/photos/BWPlus_Ivywall_22_Banana_Cocktail.jpg",   css:"top:37%;left:25%;width:clamp(110px,13vw,230px);height:clamp(160px,34vh,360px);z-index:7;", ox:80,  oy:-10, tag:"Calamansi <b>&amp; coconut</b>" },
    { img:"/photos/BWPlus_Ivywall_07_Restaurant.jpg",        css:"bottom:7%;left:3%;width:clamp(170px,22vw,380px);height:clamp(120px,25vh,260px);z-index:4;", ox:110, oy:-90, tag:"" },
    { img:"/photos/BWPlus_Ivywall_19_Alon_Dining_Alt.jpg",   css:"bottom:10%;left:37%;width:clamp(150px,17vw,300px);height:clamp(110px,22vh,240px);z-index:6;",ox:0,   oy:-110,tag:"Alon <b>all-day</b>" },
    { img:"/photos/BWPlus_Ivywall_09_Rooftop_Bar_Night.jpg", css:"top:30%;right:4%;width:clamp(180px,22vw,390px);height:clamp(170px,37vh,400px);z-index:5;", ox:-110,oy:0,   tag:"Teraza <b>rooftop</b>" },
    { img:"/photos/BWPlus_Ivywall_12_Morning_Teraza.jpg",    css:"top:7%;left:37%;width:clamp(140px,17vw,300px);height:clamp(100px,22vh,230px);z-index:4;",  ox:0,   oy:110, tag:"Morning <b>teraza</b>" },
    { img:"/photos/BWPlus_Ivywall_15_Agos_Pool_Bar.jpg",     css:"bottom:8%;right:6%;width:clamp(140px,16vw,290px);height:clamp(110px,21vh,230px);z-index:6;",ox:-100,oy:-90, tag:"Agos <b>pool bar</b>" },
    { img:"/photos/BWPlus_Ivywall_08_Bar_Lounge.jpg",        css:"top:48%;right:25%;width:clamp(120px,15vw,260px);height:clamp(130px,24vh,280px);z-index:4;",ox:-80, oy:-10, tag:"" },
    { img:"/photos/BWPlus_Ivywall_18_Bar_Lounge_Alt.jpg",    css:"top:14%;left:24%;width:clamp(110px,14vw,240px);height:clamp(90px,19vh,210px);z-index:3;",  ox:90,  oy:80,  tag:"" },
    { img:"/photos/BWPlus_Ivywall_17_Facade_Teraza.jpg",     css:"top:40%;left:43%;width:clamp(120px,15vw,260px);height:clamp(90px,18vh,200px);z-index:2;",  ox:0,   oy:10,  tag:"" },
    { img:"/photos/ivy_sop_boodle_beach.jpg",                css:"bottom:22%;left:16%;width:clamp(140px,17vw,300px);height:clamp(150px,28vh,320px);z-index:7;",ox:90, oy:-60, tag:"Beach <b>boodle</b>" }
  ];
  function clamp(v,a,b){return v<a?a:v>b?b:v;}
  function lerp(a,b,t){return a+(b-a)*t;}
  function eo(t){return 1-Math.pow(1-t,3);}
  var built=false, sectionEl=null, slideLayer=null, slideEls=[], imgEls=[], headEl=null, cueEl=null, ctaEl=null, raf=0, sIdx=0, sTimer=0;
  function build() {
  ...
  function update() {
    ...
    if (slideLayer) slideLayer.style.opacity = clamp(1-(p-0.04)/0.16,0,1).toFixed(3);
    var colOp = clamp((p-0.10)/0.18,0,1);
    for (var i=0;i<imgEls.length;i++){
      var rp = eo(clamp((p-0.10 - i*0.012)/0.46,0,1));
      var sc = lerp(1.5,1,rp);
      var tx = lerp(IMAGES[i].ox,0,rp), ty = lerp(IMAGES[i].oy,0,rp);
      imgEls[i].style.opacity = colOp.toFixed(3);
      imgEls[i].style.transform = "translate("+tx.toFixed(1)+"px,"+ty.toFixed(1)+"px) scale("+sc.toFixed(3)+")";
    }
    if (headEl){ var h=clamp((p-0.34)/0.22,0,1); headEl.style.opacity=h.toFixed(3); headEl.style.transform="translateY(-50%) scale("+lerp(1.08,1,eo(h)).toFixed(3)+")"; }
    if (cueEl) cueEl.style.opacity = clamp(1-p/0.10,0,1).toFixed(3);
    if (ctaEl) ctaEl.style.opacity = clamp((p-0.74)/0.18,0,1).toFixed(3);
  }
  function onScroll(){ if(raf) return; raf=requestAnimationFrame(function(){ raf=0; update(); }); }
...
(function(){
 ...
 function fz(){ if(!footEl||!footEl.isConnected){ var sec=document.getElementById("iwend"); footEl=sec?sec.querySelector(".ie-footwrap"):null; } if(!footEl) return;
   var vh=window.innerHeight, r=footEl.getBoundingClientRect();
   var prog=1-(r.bottom-vh)/(vh*0.62); prog=prog<0?0:(prog>1?1:prog);
   footEl.style.transform="scale("+(1-prog*0.10).toFixed(3)+")";
   footEl.style.borderRadius=(prog*26).toFixed(1)+"px";
   footEl.style.setProperty("--bd",(prog*3).toFixed(1)+"px");
 }
 ...
 function build(){
   ...
   var AERIAL="/photos/BWPlus_Ivywall_14_Aerial_DJI0002.jpg";
   s.innerHTML=
   '<div class="ie-rip top">'+RIP_SAGE+'</div>'+
   ...
   '</div></div>'+
   '<div class="ie-stay">'+
     '<div class="ie-rip">'+RIP_CREAM+'</div>'+
     '<div class="ie-cta"><div class="bgimg"><img src="'+AERIAL+'" alt="Aerial of The Ivywall, Alona Beach"></div>'+
       ...
       '</div></div>'+
     '<div class="ie-footwrap"><div class="ie-foot"><div class="bgimg"><img src="'+AERIAL+'" alt="The Ivywall from above"></div>'+
       '<div class="cols">'+
   ...
   footEl=s.querySelector(".ie-footwrap"); fz();
   return true;
 }
 function enforce(){ if(!document.getElementById("iwend")){ built=false; build(); } hideFooter(); }
 function init(){ if(!build()){ setTimeout(init,120); return; }
   window.addEventListener("scroll", onScrollFz, {passive:true}); window.addEventListener("resize", onScrollFz, {passive:true});
   try{ var mo=new MutationObserver(function(){enforce();}); mo.observe(document.querySelector("main")||document.body,{childList:true}); setTimeout(function(){try{mo.disconnect();}catch(e){}},9000);}catch(e){}
 }
