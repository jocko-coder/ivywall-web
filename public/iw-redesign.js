...
(function () {
  ...
  var built=false, sectionEl=null, headEl=null, deckEl=null, stageEl=null, cardEls=[], raf=0;
  function build() {
    ...
    var n=ROOMS.length;
    var cards=ROOMS.map(function(r,i){
      return '<div class="rs-card" data-i="'+i+'">'+
    ...
    s.innerHTML='<div class="rs-pin"><div class="rs-head"><div class="eyebrow">'+EYEBROW+'</div><h2>'+HEAD+'</h2></div>'+
      '<div class="rs-deck"><div class="rs-track"><div class="rs-stage">'+cards+'</div></div></div></div>'+
      '<div class="rs-foot"><a href="'+ROOMS_HREF+'">See all rooms <span>→</span></a></div>';
    parentRef.insertBefore(s, beforeRef);
    sectionEl=s; headEl=s.querySelector(".rs-head"); deckEl=s.querySelector(".rs-track"); stageEl=s.querySelector(".rs-stage"); cardEls=[].slice.call(s.querySelectorAll(".rs-card")); built=true; measure();
    return true;
  }
  function measure(){
  ...
  function rupd(){
    if(!deckEl||!stageEl||!cardEls.length) return;
    if(window.innerWidth<=760){ cardEls.forEach(function(c){c.style.transform="";c.style.filter="";c.style.opacity="";c.style.zIndex="";}); return; }
    var vh=stageEl.clientHeight||window.innerHeight;
    var total=deckEl.offsetHeight - vh; if(total<=0) total=1;
    var scrolled=-deckEl.getBoundingClientRect().top; if(scrolled<0)scrolled=0; if(scrolled>total)scrolled=total;
    var n=cardEls.length;
    var p=scrolled/total*(n-1); if(p<0)p=0; if(p>n-1)p=n-1;
    for(var i=0;i<n;i++){
      var t=p-i, ty, sc, br, op, z, fl;
      if(t<=0){
        var tt=t<-1?-1:t;
        ty=(-tt)*vh*0.62; sc=1; br=18; op=1;
        z=100+Math.round((1+tt)*8); fl="none";
      } else {
        var r=t>3.4?3.4:t;
        ty=-(r*vh*0.125); sc=1-r*0.092; br=18+r*26;
        op=r>2.8?Math.max(0,1-(r-2.8)/0.6):1;
        z=100-Math.round(r*10);
        fl="brightness("+(1-r*0.13).toFixed(3)+")"+(r>0.3?" blur("+Math.min(r*1.1,3.2).toFixed(2)+"px)":"");
      }
      var c=cardEls[i];
      c.style.transform="translateY(-50%) translateY("+ty.toFixed(1)+"px) scale("+sc.toFixed(3)+")";
      c.style.borderRadius=br.toFixed(0)+"px";
...
(function () {
  ...
  function build() {
    ...
    sTimer=setInterval(function(){ sIdx=(sIdx+1)%slideEls.length; slideEls.forEach(function(el,i){ el.classList.toggle("on", i===sIdx); }); }, 2600);
